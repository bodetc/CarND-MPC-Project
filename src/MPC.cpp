...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  result[0]=solution.x[delta_start] / Lf; // steering
  result[1]=solution.x[a_start]; // throttle
  for(int i=0; i<N; i++) {
    result[2+i]=solution.x[x_start+i]; // mpc_x
    result[2+N+i]=solution.x[y_start+i]; // mpc_y
  }
